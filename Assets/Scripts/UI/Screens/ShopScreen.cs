...
namespace UI.Screens
{
    public class ShopScreen : Bnjwkenjkrnjwe
    {
        ...
        public void SelectShip(int key)
        {
            ...
            buttonText.text = _bnjkwenrjkwe.BoughtShips.Contains(key) ? "SELECT" : "BUY";
        }
        ...
        public void OnButtonClick()
        {
            ...
            {
                _bnjkwenrjkwe.SelectedShip = _selectedKey;
            }
            else
            {
                if (_bnjkgnrjkweqw.Vbnjekwnrkjw[_selectedKey] > _bnjkwenrjkwe.Score)
                {
                    StartCoroutine(WrongBalance());
                    return;
                }

                _bnjkwenrjkwe.Score -= _bnjkgnrjkweqw.Vbnjekwnrkjw[_selectedKey];
                _bnjkwenrjkwe.BoughtShips = _bnjkwenrjkwe.BoughtShips
        ...
        {
            var boughtShips = _bnjkwenrjkwe.BoughtShips;
            
            for (int i = 0; i < shipCosts.Length; ++i)
            {
                if (i == _bnjkwenrjkwe.SelectedShip)
                {
            ...
            balanceText.text = $"BALANCE: {_bnjkwenrjkwe.Score}";
        }
